...
function getMe(){
  ...
  // Тест в браузере без Telegram.
  // Чтобы проверять переподключение, id должен ПЕРЕЖИВАТЬ обновление страницы.
  // Способ 1: явно задать в URL — ?room=TEST1&player=1  (удобно для 2 окон)
  const p = new URLSearchParams(location.search);
  const explicit = p.get("player");
  if(explicit){
    const id = 900000000 + Number(explicit); // стабильный id из номера игрока
    return { id: id, name: "Игрок-" + explicit };
  }

  // Способ 2 (если player не задан): стабильный id в localStorage,
  // уникальный по КЛЮЧУ вкладки. Переживает обновление.
  let tabKey = sessionStorage.getItem("tabKey");
  if(!tabKey){ tabKey = "tab" + Math.floor(Math.random()*1e6); sessionStorage.setItem("tabKey", tabKey); }
  let id = localStorage.getItem("testId_" + tabKey);
  if(!id){ id = 800000000 + Math.floor(Math.random()*1e6); localStorage.setItem("testId_" + tabKey, id); }
